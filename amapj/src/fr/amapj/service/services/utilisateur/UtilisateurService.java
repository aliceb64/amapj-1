...
/**
 * Permet la gestion des utilisateurs en masse ou du changement de son état
 * 
 */
public class UtilisateurService {
	...
	/**
	 * Permet de charger la liste de tous les utilisateurs dans une transaction en
	 * lecture
	 */
	@DbRead
	...
	public UtilisateurInfo createNewUser(UtilisateurDTO utilisateurDTO, boolean generatePassword, boolean sendMail) {
		...
		if (generatePassword == false) {
			new PasswordManager().setUserPassword(u.getId(), utilisateurDTO.password.trim());
			return res;
		}
	...
	/**
	 * Permet de supprimer un utilisateur Ceci est fait dans une transaction en
	 * ecriture
	 */
	@DbWrite
	...
	private void checkRole(List<RoleList> rls, RoleList rl) {
		if (rls.contains(rl)) {
			throw new UnableToSuppressException(
					"Cet utilisateur est " + rl + ". Enlevez lui ce rôle avant de le supprimer.");
		}
	...
	public List<Utilisateur> getUtilisateurs(boolean includeInactif) {
		...
		if (includeInactif) {
			...
			q = em.createQuery(
					"select u from Utilisateur u " + "where u.etatUtilisateur=:etat " + "order by u.nom,u.prenom");
	...
	public EnvoiMailDTO getEnvoiMailDTO() {
		...
		Query q = em.createQuery(
				"select u from Utilisateur u where u.etatUtilisateur=:etat and u.password is null order by u.nom,u.prenom");
		q.setParameter("etat", EtatUtilisateur.ACTIF);
		List<Utilisateur> us = q.getResultList();
	...
	/**
	 * Envoi un e mail a chauqe utilisateur, dans une transaction separée pour
	 * chaque utilisateur
	 * 
		...
		for (EnvoiMailUtilisateurDTO dto : envoiMail.utilisateurs) {
			if (dto.sendMail) {
				...
				try {
					...
					// positionne le mot de passe pour cet utilisateur dans une transaction en
